    ...
    <div className="space-y-8 animate-in-fade">
      <div className="flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3 px-4 py-2 rounded-xl bg-muted border border-border">
           ...
           <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Real-time Data Sync Active</span>
        </div>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {reportTypes.map((report) => (
          <div key={report.title} className="p-8 rounded-[2.5rem] bg-card border border-border shadow-sm hover:border-primary/50 transition-all hover:shadow-2xl hover:shadow-primary/5 group">
             <div className="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center mb-6 group-hover:bg-primary transition-all">
                <report.icon className="h-7 w-7 text-primary group-hover:text-primary-foreground transition-all" />
             </div>
             <h3 className="text-xl font-black text-foreground mb-2">{report.title}</h3>
             <p className="text-sm font-bold text-muted-foreground leading-relaxed mb-8">{report.description}</p>
             <button 
               onClick={() => handleExport(report.id)}
               className="w-full py-3 rounded-xl bg-muted text-[10px] font-black uppercase tracking-widest text-muted-foreground hover:bg-foreground hover:text-background dark:hover:bg-primary transition-all flex items-center justify-center gap-2"
             >
