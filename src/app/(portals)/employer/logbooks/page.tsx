    ...
    <div className="space-y-12 max-w-7xl mx-auto pb-24 animate-in-fade">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-border pb-10">
        ...
        <div className="flex bg-muted p-1.5 rounded-2xl border border-border shadow-inner">
           <button className="px-6 py-2.5 rounded-xl bg-card shadow-lg text-[10px] font-black uppercase tracking-widest transition-all text-foreground">Pending Matrix</button>
           <button className="px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 hover:text-foreground transition-all">Audit Archive</button>
        </div>
      </div>
                ...
                return (
                  ...
                  >
                    <div className={cn(
                      ...
                      selectedStudentId === trainee.studentId ? "bg-white/10" : "bg-muted text-muted-foreground/40"
                    )}>
        ...
        <div className="lg:col-span-3">
           {selectedStudentId ? (
             <div className="space-y-10 animate-in-fade">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-border pb-8">
                   <div className="space-y-2">
                     ...
                     <h3 className="text-3xl font-black flex items-center gap-4 tracking-tighter uppercase text-foreground">
                        {selectedStudent?.studentName}
                     </h3>
                   </div>
                   <div className="flex items-center gap-4">
                      <div className="px-5 py-2.5 rounded-xl bg-muted/50 border border-border/60 flex items-center gap-3">
                         <Zap className="h-4 w-4 text-amber-500 fill-amber-500" />
                         <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Total Logs: {selectedStudent?.logs.length}</span>
                      </div>
                      <button className="h-12 w-12 rounded-xl bg-card border border-border/60 flex items-center justify-center hover:bg-muted transition-all shadow-sm">
                        ...
                        <div key={entry.id} className="bg-card border border-border/60 rounded-[2.5rem] p-8 lg:p-10 shadow-3xl hover:shadow-4xl transition-all group overflow-hidden relative">
                           ...
                           <div className="flex flex-col md:flex-row md:items-start justify-between gap-10 relative z-10">
                              <div className="flex items-start gap-8 flex-1">
                                 <div className="w-16 h-16 rounded-[1.5rem] bg-muted border border-border/40 flex flex-col items-center justify-center shrink-0 group-hover:bg-blue-600 transition-all group-hover:border-blue-500 group-hover:scale-105 shadow-sm">
                                    <span className="text-[10px] font-black uppercase text-muted-foreground/40 group-hover:text-blue-100 leading-none mb-1">
                                      {new Date(entry.date).toLocaleDateString('en-US', { month: 'short' })}
                                    </span>
                                    <span className="text-2xl font-black leading-none group-hover:text-white text-foreground">
                                      {new Date(entry.date).getDate()}
                                    </span>
                                 </div>
                                 <div className="space-y-4">
                                    <div className="flex items-center gap-4">
                                       <div className="flex items-center gap-2 px-3 py-1 bg-card rounded-lg border border-border/40 shadow-sm">
                                          <Clock className="h-4 w-4 text-blue-600" />
                                          <span className="text-sm font-black tracking-tighter text-foreground uppercase">{entry.hours} Hours</span>
                                       </div>
                                       <span className={cn(
                                    ...
                                    <div className="p-6 rounded-[1.5rem] bg-muted/30 border-l-4 border-blue-600 shadow-inner">
                                 ...
                                 {entry.status === 'PENDING' ? (
                                   <div className="flex items-center gap-3 w-full md:w-auto">
                                      ...
                                      <button
                                        ...
                                        disabled={isProcessing === entry.id}
                                        className="h-14 px-6 rounded-2xl bg-card border border-border/60 text-[10px] font-black hover:bg-rose-500 hover:text-white hover:border-rose-500 transition-all flex items-center justify-center gap-2 uppercase tracking-widest disabled:opacity-30 text-muted-foreground/60"
                                      >
                                   ...
                                   <div className="flex flex-col items-end gap-3">
                                      {entry.feedback && (
                                        <div className="flex items-center gap-2.5 px-5 py-3 rounded-xl bg-muted text-muted-foreground/60 text-[10px] font-black uppercase tracking-widest border border-border/40">
                                          <MessageSquare className="h-3.5 w-3.5" />
                                          {entry.feedback}
                                        </div>
                                      )}
                                      <button className="h-12 w-12 flex items-center justify-center rounded-2xl bg-muted border border-border/40 hover:bg-primary hover:text-primary-foreground transition-all group/btn shadow-sm">
                                         <MoreVertical className="h-5 w-5 text-muted-foreground/40 group-hover/btn:text-white" />
                                      </button>
                                   </div>
                                 )}
                              </div>
             ...
             <div className="h-[70vh] flex flex-col items-center justify-center text-center bg-card border border-border/60 rounded-[4rem] p-20 shadow-4xl relative overflow-hidden group">
                ...
                <div className="w-28 h-28 rounded-[2rem] bg-muted flex items-center justify-center mb-10 shadow-3xl ring-8 ring-blue-600/5">
                   <UserIcon className="h-12 w-12 text-muted-foreground/40 group-hover:text-blue-600 transition-colors" />
                </div>
                <h3 className="text-4xl font-black mb-4 tracking-tighter uppercase text-foreground">
                  Initialize <span className="text-blue-600">Review</span>
                </h3>
