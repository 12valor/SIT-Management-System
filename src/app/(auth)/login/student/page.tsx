    ...
    <div className="flex min-h-screen overflow-hidden bg-white">
      ...
      <div className="relative hidden lg:flex lg:w-3/5 xl:w-[60%] bg-slate-50 flex-col justify-between p-12 overflow-hidden border-r border-slate-200">
        ...
        <div className="relative z-10">
           <div className="inline-flex items-center gap-4 px-5 py-3 rounded-xl bg-white border border-slate-200 shadow-sm">
              ...
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest font-heading font-black">Secure Student Node</p>
           </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center p-8 sm:p-12 lg:p-24 bg-white relative z-20">
        <div className="max-w-md mx-auto w-full lg:mx-0">
          <div className="mb-10">
            <Link href="/login" className="inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-slate-400 hover:text-primary transition-colors mb-10 font-heading">
              <ArrowLeft className="h-3 w-3" /> Back to role selection
            </Link>
            <h2 className="text-4xl font-bold tracking-tight text-slate-900 mb-3 font-heading uppercase">Student Login</h2>
            <p className="text-base text-slate-500 font-medium font-sans">Authorized access for registered TUP-V students.</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <div className="space-y-2">
              <label htmlFor="email" className="text-[10px] font-bold uppercase tracking-widest text-slate-500 font-heading">University Email</label>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <Link href="#" className="text-[10px] font-bold text-primary hover:underline uppercase tracking-widest font-heading">Forgot?</Link>
              </div>
              <div className="relative group">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full h-14 bg-primary text-primary-foreground text-xs font-bold uppercase tracking-[0.2em] rounded-xl hover:shadow-lg hover:shadow-primary/30 transition-all flex items-center justify-center gap-3 disabled:opacity-50 font-heading"
            >
              {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Sign In Portal"}
            </button>
          </form>

          <footer className="mt-12 pt-8 border-t border-slate-100 flex items-center justify-between">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-heading">No account?</span>
            <Link href="/signup/student" className="text-[10px] font-bold text-primary uppercase tracking-widest hover:underline underline-offset-4 font-heading">
              New Registration
            </Link>
          </footer>
        </div>
